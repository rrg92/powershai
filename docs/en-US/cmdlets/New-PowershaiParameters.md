...
When enabled, messages generated via stream are not shown as they are produced, and only the final result is returned.
...
### -ShowTokenStats
At the end of each message, it displays the consumption statistics, in tokens, returned by the API
...
When using tool calling, this parameter limits how many tools without a sequence that resulted in an error can be called.  
The considered error is the exception triggered by the configured script or command.
...
Controls the number of messages in the current chat context. When this number exceeds, Powershai automatically clears the oldest messages.
...
Function used to format the objects passed via pipeline
...
To use it you must know the details of the provider's implementation and how its API works!
