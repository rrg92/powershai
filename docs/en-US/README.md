...
You can chat with GPT-4o-mini, Gemini Flash, Llama 3.3, etc., using your own tokens from these services.  
...
To get started, you must choose a provider and configure the authentication for it.  
...
See the [providers documentation](providers/) to learn more about the architecture and operation.
...
I recommend reading this after you have followed at least [example 0001], so that you become more familiar with its use.
...
We call these commands the Powershai `global commands`, as they are not commands for a specific provider.

For example: `Get-AiProvider`, `Send-PowershaiChat`, `New-PowershaiChat`.  
Providers also export commands, which will generally have a provider name. Consult the provider's documentation to learn more about the exported command pattern.  
...
Provider documentation is always available via help `about_Powershai_ProviderName_Topic`.  
...
Chats are the main starting point and allow you to chat with the various LLMs provided by providers.  
...
The easiest way to start the conversation is using the `Send-PowershaiChat` command or the `ia` alias:
...
You can use the pipeline to directly throw the results of other commands as context for the AI:
...
The difference in the above command is that the AI will be called 10 times, one for each number.  
...
# After the command above runs, just export!
