...
Vous pouvez appeler les commandes à la fois dans des scripts et en ligne de commande.  

Il existe plusieurs commandes qui permettent des conversations avec les LLMs, d'appeler des espaces de Hugging Face, Gradio, etc.  
...
Ce module est idéal pour intégrer des commandes PowerShell avec vos LLMs préférés, tester des appels, des POC, etc.  
...
Set-OpenaiToken # configure un jeton pour OpenAI (nécessite de le faire une seule fois)
...
Set-GoogleApiKey # configure un jeton pour Google Gemini (nécessite de le faire une seule fois)
...
Les exemples ci-dessus ne sont qu'une petite démonstration de la simplicité d'utilisation de l'IA dans votre PowerShell et de l'intégration avec pratiquement toutes les commandes!
[Explorez plus en profondeur dans la documentation](docs/pt-BR)

## Installation

Toute la fonctionnalité est dans le répertoire `powershai`, qui est un module PowerShell.  
...
# Importer depuis le chemin spécifique !
Import-Module .\powershai
```
...
_Traduit automatiquement en utilisant PowershAI et l'IA_
