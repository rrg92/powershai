<#
	HTTP BASE functions
		...
		This is basic HTTP functions used by other cmdlets in this provider. 
		This functions implements all HTTP basic operations to cover most scenarios, like file uploads (multpart-form), Server Side Events (sse), etc.
		...
		The functions are:
			...
			- Receive-HttpRequest   
				Receive reseponse from previous request created with Start-HttpRequest
				
			- Invoke-Http 
				Wrapper for Start-HttpRequest + Receive-HttpRequest.  
				This like using Invoke-WebRequest. It will start request and receive up to the end (by default).
#>
...
function NewHttpChangeReq {
	...
	$ParentParams = $MyParent.Parameters;
	@($ParentParams.keys) | %{ $CmdLetParams[$_] = Get-Variable $_ -EA SilentlyContinue }
	...
	function ChangeReq {
		...
		$null = & PowershaiHttpFilter $Params	
	}
}
...
Function Start-HttpRequest {
	<#
		.SYNOPSIS 
			Define a new http request to be used with Get-HttpResponse
		
		.DESCRIPTION 
			Cria uma requisição http, definido url e parâmetros (e dados a serem enviados).  
			Apenas cria a requisição, mas não recebe nenhuma resposta. PAra obter a resposta, use Get-HttpResponse, passando o objeto retornado por este cmdlet.
			
			Opcionalmente, você pode definir uma função global chamada PowershaiHttpFilter.  
			Essa função te permite manipular os objetos internos de requisição, permitindo um maior controle.
			A função receberá um parâmetro com informações úteis para filtro.
			Consulte a função NewHttpChangeReq para mais detalhes dos parâmetros enviados.
			Procure por ChangeReq neste script para ver os eventos disponíveis.
	#>
	[CmdLetBinding()]
	param(
	...
	# Logic to invoke user custom function to change request!
	$DebugVar = @{};
	...
	# give change to change parameters!
	$CmdLetParams = @{}
	@($PsBoundParameters.keys) | %{ $CmdLetParams[$_] = Get-Variable $_ }
